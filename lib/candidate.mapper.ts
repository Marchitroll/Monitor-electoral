...
const PHOTO_BY_SLUG: Record<string, string> = {
  "alex-gonzales-castillo": "/images/candidatos/alex-gonzales.jpg",
...
const PARTY_LOGO_BY_SLUG: Record<string, string> = {
  "alex-gonzales-castillo": "/images/logos/alex-gonzales-partido-democrata-verde.jpg",
  ...
  "francisco-ernesto-diez-canseco-tavara": "/images/logos/francisco-diez-canseco-partido-politico-peru-accion.jpg",
  ...
  "jose-daniel-williams-zapata": "/images/logos/jose-williams-avanza-pais-partido-de-integracion-social.jpg",
  ...
  "maria-soledad-perez-tello-de-rodriguez": "/images/logos/maria-perez-primero-la-gente-comunidad-ecologia-libertad-y-progreso.jpg",
...
function isCandidateFuente(value: unknown): value is CandidateFuente {
...
export function isCandidateRaw(value: unknown): value is CandidateRaw {
  ...
  if (
    ...
    !item.fuentes.every(isCandidateFuente) ||
...
export function toCandidate(raw: CandidateRaw): Candidate {
  return {
    id: raw.id,
    slug: raw.slug,
    nombre: raw.nombre,
    partido: raw.partido,
    foto: getAssetPathFromSlug(PHOTO_BY_SLUG, raw.slug, PHOTO_FALLBACK),
    logoPartido: getAssetPathFromSlug(PARTY_LOGO_BY_SLUG, raw.slug, PARTY_LOGO_FALLBACK),
    calificacion: normalizeCalificacion(raw.calificacion),
    resumen_polemicas: raw.resumen_polemicas,
    fuentes: raw.fuentes,
    tags: raw.tags,
  };
}
...
export function parseCandidates(raw: unknown): Candidate[] {
  if (!Array.isArray(raw)) {
    console.error("[candidates] invalid data source: root is not an array");
    return [];
  }

  return raw
    .map((candidate, index) => {
      if (!isCandidateRaw(candidate)) {
        console.warn(`[candidates] invalid candidate at index: ${index}`, candidate);
        return null;
      }
